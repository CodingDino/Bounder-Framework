...
{
	public bool m_useLocalAssetPath;
	...
	public static bool Iniitalized
	{
		get {
			#if UNITY_EDITOR
			return SimulateAssetBundleInEditor || m_AssetBundleManifest != null;
			#else
	...
	{
		Debug.Log("AssetBundleManager:Initialise()");
	...
	static protected void LoadDependencies(string assetBundleName)
	{
		...
		{
			dependencies = m_Dependencies[assetBundleName];
		}
		for (int i = 0; i < dependencies.Length; i++)
		{
...
//				Log(LogType.Info, "<color=green>Asset bundle already processed</color>: " + assetBundleName);
	...
	/// </summary>
	static public void UnloadAssetBundle(string assetBundleName, bool ignoreReferenceCount = false)
	{
		#if UNITY_EDITOR
		...
		UnloadAssetBundleInternal(assetBundleName, ignoreReferenceCount);
		UnloadDependencies(assetBundleName, ignoreReferenceCount);
	}

	static public void UnloadAllAssetBundles()
	{
		List<string> toUnload = new List<string>();
		foreach (var entry in m_LoadedAssetBundles)
		{
	...
	static protected void UnloadDependencies(string assetBundleName, bool ignoreReferenceCount = false)
	{
		string[] dependencies = null;
		if (!m_Dependencies.TryGetValue(assetBundleName, out dependencies))
			return;
		...
		foreach (var dependency in dependencies)
		{
			UnloadAssetBundleInternal(dependency, ignoreReferenceCount);
		}

		m_Dependencies.Remove(assetBundleName);
	}

	static protected void UnloadAssetBundleInternal(string assetBundleName, bool ignoreReferenceCount = false)
	{
		string error;
		...
		if (ignoreReferenceCount || --bundle.m_ReferencedCount == 0)
		{
			bundle.OnUnload();
